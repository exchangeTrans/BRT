...
export default {
  ...
  mounted(){
    ...
    this.getTradeInfoSymbolBalance(symbolCode);
  ...
  methods:{
    ...
    // /trade/getTradeInfoSymbolBalance

    getTradeInfoSymbolBalance(symbolType){
        ...
        let that = this;
        this.$request({
            url:'trade/getTradeInfoSymbolBalance',
            method:'post',
            params:postData
        }).then((res)=>{
            if (res.result.returnCode.toString() === "0") {
                ...
                setTimeout(() => {
                    that.getTradeInfoSymbolBalance()
                }, 5000);
    ...
    getTradeInfoSymbolBalance_refresh(times,symbolType){
        if(this.closeRefresh) return
        if(times===0) return
        if(!symbolType){
            symbolType = this.KLineTradingPair.name;
        ...
        this.$request({
            ...
            if (res.result.returnCode.toString() === "0") {
                ...
                // that.upDataRange_BRT(symbolPrice)
                setTimeout(() => {
                    that.getTradeInfoSymbolBalance_refresh(times-1)
                }, 1000);
                // console.log(Number(usdtBalance)) 
                // this.$emit('transferInAmountSuccess')
            }else{
    ...
    getTradeInfo_refresh(times,isChangeFlag,symbolType){
        ...
        this.$request({
            ...
            if (res.result.returnCode.toString() === "0") {
                ...
                // that.upDataRange_BRT(symbolPrice)
                setTimeout(() => {
                    that.getTradeInfo_refresh(times-1,isChangeFlag)
                }, 1000);
                // console.log(Number(usdtBalance)) 
                // this.$emit('transferInAmountSuccess')
            }else{
