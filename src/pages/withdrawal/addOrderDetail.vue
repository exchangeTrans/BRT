<template>
    <view id="addOrderDetail" class="addOrderDetail">
        <!-- @headertap="headertap" -->
        <pageHeader :headerOptions="headerOptions"
                    
                    class="appHeader"></pageHeader>
        <scroll-view class="addOrderDetail-wrap" :scroll-y="true" @touchmove.stop.prevent="moveHandle">
            <view class="addOrderDetailContent">
                <view class="detailItem detailItem1">
                    <view class="number">{{amount}} {{symbolTypeText}}</view>
                </view>
                <view class="detailItem detailItem2">
                    <view class="detailItem_left">{{$t('addOrderDetail').type}}</view>
                    <view class="detailItem_right">{{operationTypeText}}</view>
                </view>
                <view class="detailItem detailItem3">
                    <view class="detailItem_left">{{$t('addOrderDetail').state}}</view>
                    <view class="detailItem_right">{{stateTypeText}}</view>
                </view>
                <view class="detailItem detailItem4">
                    <view class="detailItem_left">{{$t('addOrderDetail').serviceCharge}}</view>
                    <view class="detailItem_right">{{fee}} {{symbolTypeText}}</view>
                </view>
                <view class="detailItem detailItem5">
                    <view class="detailItem_left">{{$t('addOrderDetail').addressFrom}}</view>
                    <view class="detailItem_right">{{addressFrom}}</view>
                </view>
                <view class="detailItem detailItem5">
                    <view class="detailItem_left">{{$t('addOrderDetail').withdrawAddress}}</view>
                    <view class="detailItem_right">{{memo}}</view>
                </view>
                
                <view class="detailItem detailItem6">
                    <view class="detailItem_left">{{$t('addOrderDetail').time}}</view>
                    <view class="detailItem_right">{{createTime}}</view>
                </view>
                <view v-if='symbolTypeText==="BRT"' class="addOrderDetailTip">{{$t('addOrderDetail').tip}}</view>
            </view>
...
<script >
// src="@/script/addOrderDetail/addOrderDetail.js"
            ...
            data () {
                return {
                    ...
                    headerOptions: {
                        show: true,
                        isAllowReturn: true,
                        text: this.$t('addOrderDetail').title,
                        rightItem: {
                            // type: "text",
                            // text: "须知&反馈",
                            // style: {
